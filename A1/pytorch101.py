...
def multiples_of_ten(start, stop):
  ...
  x = torch.tensor([elem for elem in range(start, stop + 1) if elem % 10 == 0],
                    dtype = torch.float64)
...
def reverse_rows(x):
  ...
  sp = x.shape
  idx = [sp[0] - i - 1 for i in range(sp[0])]
  y = x[idx]
...
def reshape_practice(x):
  ...
  a, b = x.reshape(6, 4)[:3, :], x.reshape(6, 4)[3:, :]
  y = torch.zeros(6, 4, dtype = int)
  y[::2,] = a
  y[1::2,] = b
  y = y.reshape(3, 8)
...
def normalize_columns(x):
  ...
  mu = 1 / x.shape[0] * x.sum(dim = 0) # [1, N]
  sigma = torch.sqrt(1 / (x.shape[0] - 1) * ((x - mu) * 
                      (x - mu)).sum(dim = 0))
  y = (x - mu) / sigma
