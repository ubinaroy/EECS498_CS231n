...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      temp = torch.mul((x_train[i] - x_test[j]),
                              (x_train[i] - x_test[j]))
      dists[i][j] = temp.view(-1, 1).sum(dim = 0)
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    temp = torch.mul(x_test.view(num_test, -1) - x_train[i].view(1, -1), 
                      x_test.view(num_test, -1) - x_train[i].view(1, -1))
    dists[i, ] = temp.view(num_test, -1).sum(dim = 1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = (x_train.view(num_train, -1) ** 2).sum(dim = 1, keepdim = True)\
            + (x_test.view(num_test, -1) ** 2).sum(dim = 1, keepdim =True).t()\
            - 2 * torch.mm(x_train.view(num_train, -1), x_test.view(num_test, -1).t())
...
def predict_labels(dists, y_train, k=1):
  ...
  # Attention, the "DISTS" holds num_train{rows} and num_test{cols}
  # We use the nearest distance in each cols to rep the test data belong to 
  # which labels, train reps the diff labels.

  idx = dists.topk(k, dim = 0, largest = False)[1]
  idx = idx.T

  labels = y_train[idx] # each row reps the match label of each test example
                        # and it should be [test_num X K] dimention

  y_pred = labels.mode().values
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for n in range(num_folds):
        ...
        x_train, y_train = torch.cat(x_train_list, dim = 0),\
                                    torch.cat(y_train_list, dim = 0)
