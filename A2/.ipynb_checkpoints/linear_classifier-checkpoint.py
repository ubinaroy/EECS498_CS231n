...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # Regularization
  RW = torch.sum(torch.mul(W, W)) * reg
  
  # Hinge Loss
  Delta = 1
  num_class = W.shape[1]
  num_example = X.shape[0]
  scores = torch.mm(X, W) # shape of N /times C

  correct_class_score = scores[torch.arange(num_example), y] # N
  margin = scores - correct_class_score.view(-1, 1) + 1
  margin[torch.arange(num_example), y] = 0
  
  margin[margin < 0] = 0

  loss = torch.sum(margin) / num_example + RW
  ...
  margin[margin > 0] = 1
  margin[torch.arange(num_example), y] = -1 * torch.sum(margin, dim = 1)

  dW = torch.mm(X.T, margin) / num_example + 2 * reg * W
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # W = 0.0001 * torch.randn(X_train.shape[1], y_train.shape[0], 
  #               dtype=data_dict['X_val'].dtype, device=data_dict['X_val'].device)
  # W = None
  # W = train_linear_classifier(svm_loss_vectorized, W, X_train, y_train, lr, reg, num_iters)[0]

  
  # y_train_pred = predict_linear_classifier(W, X_train)
  # y_val_pred = predict_linear_classifier(W, X_val)
