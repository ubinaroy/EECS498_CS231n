...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    max_scores = torch.max(scores, dim = 1).values.view(-1, 1)
    scores -= max_scores
    ...
    # Loss for softmax function, and probs for softmax
    probs[torch.arange(N), y] += -1 # upstream
    probs /= N
    
    # N,H.T * N,C -> H,C
    grads['W2'] = torch.mm(h1.T, probs) + 2 * reg * W2
    grads['b2'] = torch.sum(probs, dim = 0) # C,

    part_L_y1 = torch.mm(probs, W2.T) # N,C * H,C.T -> N,H

    ### 这里能不能理解为, 第一层与第二层之间, 加了个 ReLU 激活函数? 是的, 的确加了.
    part_L_y1[h1 <= 0] = 0
    # N,H * N,D -> D,H
    grads['W1'] = torch.mm(X.T, part_L_y1) + 2 * reg * W1 
    grads['b1'] = torch.sum(part_L_y1, dim = 0)
...
def nn_predict(params, loss_func, X):
  ...
  h1 = torch.mm(X, params['W1']) + params['b1']
  output = torch.mm(h1, params['W2']) + params['b2']
